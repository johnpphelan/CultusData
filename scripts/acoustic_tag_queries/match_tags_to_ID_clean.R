...
transmitter_data_thompson_two<-read_excel("data/Acoustic_tags/29375 THOMPSON RIVERS UNIVERSITY.xls", sheet = "Sensor Tags")
transmitter_data_thompson_three<-read_excel("data/Acoustic_tags/29375 THOMPSON RIVERS UNIVERSITY.xls", sheet = "Coded Tags")
...
#next sheet  
pos <- which(transmitter_data_thompson_two == "Date:", arr.ind = TRUE)
date_value <- transmitter_data_thompson_two[pos[1], pos[2]+2]
real_date <- as.Date(as.numeric(date_value), origin = "1899-12-30")
transmitter_data_thompson_two<-transmitter_data_thompson_two[3:nrow(transmitter_data_thompson_two), 8:ncol(transmitter_data_thompson_two)]
...
transmitter_data_thompson_two <- transmitter_data_thompson_two[complete.cases(transmitter_data_thompson_two),]
                                                              
                                                              

# 2.3 Thompson three data
                    
transmitter_data_thompson_three                                          

pos <- which(transmitter_data_thompson_three == "Date:", arr.ind = TRUE)
date_value <- transmitter_data_thompson_three[pos[1], pos[2]+2]
real_date <- as.Date(as.numeric(date_value), origin = "1899-12-30")
transmitter_data_thompson_three<-transmitter_data_thompson_three[3:nrow(transmitter_data_thompson_three), 8:ncol(transmitter_data_thompson_three)]
names(transmitter_data_thompson_three)<-transmitter_data_thompson_three[1,]
transmitter_data_thompson_three<-transmitter_data_thompson_three[-1,]
transmitter_data_thompson_three$date=real_date
transmitter_data_thompson_three <- transmitter_data_thompson_three[complete.cases(transmitter_data_thompson_three),]
...
transmitter_data_thompson_two <- transmitter_data_thompson_two |> 
  rename(transmitter = `VUE Tag ID\n(Freq-Space-ID)`)
transmitter_data_thompson_three <- transmitter_data_thompson_three |>
  rename(transmitter = `VUE Tag ID\n(Freq-Space-ID)`)
combined_thompson <- bind_rows(transmitter_data_thomspon_one, transmitter_data_thompson_two) |>
  distinct(transmitter, .keep_all = TRUE)  # avoid duplicates if any
combined_thompson <- bind_rows(combined_thompson, transmitter_data_thompson_three) |>
  distinct(transmitter, .keep_all = TRUE)  # avoid duplicates if any
joined_transmitters <- transmitter_data |>
  mutate(transmitter = as.character(transmitter)) |> 
  left_join(combined_thompson, by = "transmitter")

# no matches - where are these tags?
