...
These are all plots that could be easily completed - subject to how columns are structured. If there are mixes of letters and numbers, we will not be able to convert that to numeric. The following are plots of what data was converted, which can be used to check for any erroneous data entries.



```{r main_plots}

to_remove_from_plots<-c("survey_number", "time", "shift", "release_reason", "hours_worked", "shift_start", "shift_end" , "shift_start_time", "shift_end_time")
...
plots <- main_page |>
  select(-all_of(to_remove_from_plots)) |> 
