...
demo_data <- demo_data %>%
  mutate(
    # make sure 'time' is numeric (Excel serial)
    time_num = as.numeric(date_time),
    
    # convert Excel serial to POSIXct in PST
...
fish <- sheets_combined$`Fish Data` %>%
  ...
  select(-date_time) |> 
  dplyr::distinct() # are the rows distint?
...
# Prepare a named list of cleaned sheets
sheets_to_save <- list(
  "Main Data" = main_page,
...
write_xlsx(
  sheets_to_save,
  path = "./output/cleaned_creel_all_years.xlsx"
)
