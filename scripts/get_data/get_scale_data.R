...
scale_data <- scale_data |> 
  mutate(original_order = as.numeric(original_order), Waterbody = as.character(Waterbody),
         ...
         Scale_Nos = as.character(Scale_Nos), BCPAL_To_Age_YN_Filtered_by_Y_or_N = as.character(BCPAL_To_Age_YN_Filtered_by_Y_or_N),
         GPS_Point = as.character(GPS_Point), Start_Time = as.numeric(Start_Time), End_Time = as.numeric(End_Time),
         Seconds_Electricity = as.numeric(Seconds_Electricity), Pit_tag = as.numeric(Pit_tag), 
         Acoustic_Tag = as.numeric(Acoustic_Tag), Length_mm = as.numeric(Length_mm), Weight = as.numeric(Weight),
         Sex = as.character(Sex), Maturity = as.character(Maturity), Rand = as.numeric(Rand),
...
scale_500_data <- scale_500_data |> 
  mutate(Scale_Book_No = as.numeric(Scale_Book_No), Scale_Nos = as.numeric(Scale_Nos), BCPAL_To_Age_YN = as.character(BCPAL_To_Age_YN)
         , GPS_Point = as.character(GPS_Point), Start_Time = as.numeric(Start_Time),
